...
export async function runDashboard(atlas, options = {}) {
  ...
  if (!process.stdout.isTTY) {
    ...
    console.error(
      "Run this command in a terminal, not through a pipe or script.",
    );
    process.exit(1);
  }
  ...
  if (cols < 60 || rows < 15) {
    ...
    console.error(
      "Dashboard requires at least 60x15. Please resize your terminal.",
    );
    process.exit(1);
  }
  ...
  const CARD_HEIGHT = 5; // Increased for richer cards
  ...
  function renderCards() {
    ...
    // Show pagination indicator and position
    const position = filteredList.length > 0 ? `${selectedCardIndex + 1}/${filteredList.length}` : '0/0'
    const scrollHint = filteredList.length > MAX_VISIBLE_CARDS ? ' {gray-fg}↑↓ scroll{/}' : ''

    // Filter indicator in title
    const filterLabel = currentFilter === 'a' ? '{green-fg}active{/}'
      : currentFilter === 'p' ? '{yellow-fg}paused{/}'
      : currentFilter === 's' ? '{cyan-fg}stable{/}'
      : '{white-fg}all{/}'

    titleBar.setContent(
      ` {bold}ATLAS{/bold}  {gray-fg}│{/}  ${filterLabel}  {gray-fg}│{/}  ` +
      `{cyan-fg}${position}{/}${scrollHint}`
    )
  ...
  const detailCommandBar = blessed.box({
    ...
    style: { fg: "white", bg: "black" },
    content:
      " {yellow-fg}Esc{/} Back  {cyan-fg}s{/} Session  {cyan-fg}c{/} Capture  {cyan-fg}o{/} Open  {gray-fg}│{/} {yellow-fg}↑↓{/} Scroll",
  });
  ...
  const focusCommandBar = blessed.box({
    ...
    style: { fg: "gray", bg: "black" },
    content:
      " {cyan-fg}Esc{/} Exit Focus  {cyan-fg}Space{/} Pause/Resume  {cyan-fg}r{/} Reset  {cyan-fg}c{/} Capture  {cyan-fg}+/-{/} Adjust Time",
  });
  ...
  const zenCommandBar = blessed.box({
    ...
    style: { fg: "gray", bg: "black" },
    content:
      " {cyan-fg}Space{/} Pause  {cyan-fg}c{/} Capture  {cyan-fg}Esc{/} Expand  {cyan-fg}q{/} Quit",
  });
  ...
  function getStatusCategory(status) {
    if (["active", "working", "in-progress", "testing"].includes(status))
      return "a";
    if (["paused", "blocked", "waiting"].includes(status)) return "p";
    if (["stable", "complete", "released", "ready"].includes(status))
      return "s";
    return "o";
  }
  ...
  async function loadMainView() {
    try {
      ...
      titleBar.setContent(
        ` {bold}ATLAS{/bold}  {red-fg}Error: ${err.message}{/}`,
      );
    }
  ...
  async function loadDetailView(project) {
    ...
    try {
      ...
      gaugePercent = today.sessions
        ? Math.min(100, Math.round((today.sessions / 5) * 100))
        : 0;
      if (canvasSupported) {
    ...
    try {
      ...
      if (trail?.length) {
        trail.forEach((b) => {
          ...
          breadcrumbsLog.log(
            `{cyan-fg}${time}{/} ${(b.text || "").substring(0, 25)}`,
          );
        });
      } else {
  ...
  function updateFocusTimer() {
    const now = Date.now();
    const elapsed = pomodoroStart
      ? Math.floor((now - pomodoroStart) / 1000)
      : 0;
    ...
    const progress =
      pomodoroMinutes > 0
        ? Math.min(100, (elapsed / (pomodoroMinutes * 60)) * 100)
        : 0;
    const barWidth = 30;
  ...
  function updateZenDisplay() {
    ...
    const streakDays =
      pomodoroHistory.length > 0 ? Math.min(pomodoroHistory.length, 7) : 0;
    ...
    const remaining = pomodoroStart
      ? Math.max(
          0,
          pomodoroMinutes * 60 -
            Math.floor((Date.now() - pomodoroStart) / 1000),
        )
      : pomodoroMinutes * 60;
    const mins = Math.floor(remaining / 60);
    const secs = remaining % 60;
    ...
    const progress =
      pomodoroMinutes > 0 ? (elapsed / (pomodoroMinutes * 60)) * 100 : 0;
    const barWidth = 30;
    const filled = Math.round((progress / 100) * barWidth);
    const progressBar =
      "{green-fg}" +
  ...
  async function showDecisionHelper() {
    ...
    try {
      ...
      const active = projects.filter(
        (p) => getStatusCategory(p.status) === "a",
      );
      const paused = projects.filter(
        (p) => getStatusCategory(p.status) === "p",
      );
      const blocked = projects.filter((p) => p.status === "blocked");
      const stable = projects.filter(
        (p) => getStatusCategory(p.status) === "s",
      );
      ...
      if (value?.trim()) {
        try {
          ...
          statusBar.setContent(
            ` {green-fg}✓ Captured: "${value.trim().substring(0, 30)}..."{/}`,
          );
          screen.render();
          setTimeout(() => {
  ...
  screen.on("resize", () => {
    ...
    if (mode === "compact") {
      ...
      statusBar.setContent(
        " {yellow-fg}⚠ Terminal too narrow - expand for best view{/}",
      );
    }
