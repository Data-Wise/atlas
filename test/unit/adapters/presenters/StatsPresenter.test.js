...
describe('StatsPresenter', () => {
  ...
  describe('formatStatsMarkdown', () => {
    it('should include markdown header', () => {
      const stats = createMockStats()
      const output = formatStatsMarkdown(stats)

      expect(output).toContain('# Session Analytics Report')
      expect(output).toContain('**Period:** Last 7 Days')
      expect(output).toContain('**Generated:**')
    })

    it('should include summary table with markdown formatting', () => {
      const stats = createMockStats()
      const output = formatStatsMarkdown(stats)
      ...
      expect(output).toContain('| Total Sessions | 10 |')
      expect(output).toContain('| Flow Sessions | 6 (60%) |')
    })

    it('should include streak section when active', () => {
      const stats = createMockStats()
      const output = formatStatsMarkdown(stats)

      expect(output).toContain('## Streak')
      expect(output).toContain('- **Current:** 🔥 3 days')
      expect(output).toContain('- **Longest:** 10 days')
    })
    ...
    it('should include best day section', () => {
      const stats = createMockStats()
      const output = formatStatsMarkdown(stats)

      expect(output).toContain('## Best Day')
      expect(output).toContain('**Friday** with 2h')
    })

    it('should include project breakdown as markdown table', () => {
      const stats = createMockStats()
      const output = formatStatsMarkdown(stats)

      expect(output).toContain('## By Project')
      expect(output).toContain('| Project | Sessions | Time | Flow % |')
      expect(output).toContain('| atlas | 5 |')
      expect(output).toContain('| flow-cli | 3 |')
    })
    ...
    it('should include hourly distribution code block', () => {
      const stats = createMockStats({
        hourlyDistribution: [0, 0, 0, 0, 0, 0, 0, 0, 0, 10, 20, 30, 40, 30, 20, 10, 0, 0, 0, 0, 0, 0, 0, 0]
      })
      const output = formatStatsMarkdown(stats)

      expect(output).toContain('## Hourly Distribution')
      expect(output).toContain('```')
      expect(output).toContain('6am')
    })

    it('should include footer with Atlas link', () => {
      const stats = createMockStats()
      const output = formatStatsMarkdown(stats)

      expect(output).toContain('---')
      expect(output).toContain('*Generated by [Atlas]')
    })

    it('should include project filter in period when filtering', () => {
      const stats = createMockStats({ period: { projectFilter: 'atlas' } })
      const output = formatStatsMarkdown(stats)
