...
| ~~FW-6~~ | ~~`atlas plan` research grouping~~ — **CUT 2026-07-16** (adversarial review): `atlas project list --kind manuscript` (FW-7, shipped) already answers the same question; grouping the same rows inside `atlas plan` adds no new action or behavior, just a second surface for identical data | atlas | — | — | — |
...
| FW-12 | **Narrowed 2026-07-16** (adversarial review): the refresh half already shipped (#61, `scripts/sync-research-board.sh` on Monday cadence) — but it renders anyway on failure ("stale data is better than no board"), so it does NOT detect drift, it's the opposite risk. Remaining scope: just the `--dry-run` exit-code drift check | scheduler | **S** | — | nonzero exit on drift (render half already done) |
| ~~FW-13~~ | ~~`.flow/obsidian-sync.yml` → `.obs/sync.yml` migration~~ — **CUT 2026-07-16** (adversarial review): `DoctorUseCase` already treats both paths as equally valid, info-only, never gates pass/fail; docs already document dual-path acceptance as the stated policy, not a stopgap. Pure tidiness, zero functional payoff | obs + repos | — | — | — |
...
| ID | Task | Repo | Effort | Depends | Acceptance |
|----|------|------|--------|---------|-----------|
| ~~FW-26~~ | ~~Interactive HTML research dashboard~~ — **CUT 2026-07-16** (adversarial review): atlas already ships two live dashboard surfaces (Ink TUI `atlas dash`, polling live data every 5–10s; `obs research board` vault dashboard). No identified user is served by a third HTML surface that isn't already served by the other two | new | — | — | — |

---

## Sequencing & milestones
```
M1 (board truth)  : FW-1, FW-3, FW-4, FW-7  → board shows venue/progress/priority + package data  [DONE]
M2 (dashboard)    : FW-2, FW-5              → MediationVerse_Dashboard + table surface (FW-6 cut 2026-07-16 — redundant w/ FW-7)  [FW-5 DONE]
M3 (born-ready)   : FW-8, FW-9, FW-10        → new+existing projects contract-complete  [FW-9/FW-10 DONE]
M4 (retire+sched) : FW-11, FW-12, FW-14      → obs supersedes status-sync; scheduled  (FW-13 cut 2026-07-16 — no functional payoff, doctor already dual-path)
M5 (harden+docs)  : FW-15, FW-16, FW-18–FW-22  → tests + architecture/docs deep-dives  [FW-15/FW-16 DONE]
M6 (release)      : FW-24, FW-25              → installed binaries carry everything
M7 (optional)     : FW-17, FW-23              → (FW-26 cut 2026-07-16 — redundant w/ existing Ink TUI + vault board, no identified user)
```
...
**2026-07-16 status:** FW-1/3/4/5/7/9/10/15/16/17 confirmed shipped (grounded against live code, closed issue #29). FW-6/13/26 cut (adversarial value review — redundant with existing features, no functional payoff). FW-12 narrowed (refresh half shipped via #61; only the drift-detection half remains). Remaining real work: FW-2/FW-8/FW-11/FW-12/FW-14 (cross-repo/scheduler), FW-18–23/31 (docs), FW-24/25 (release).
