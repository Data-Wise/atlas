...
- **2026-07-17** — P1 scaffold started (`feature/catch-obs-bridge`, commit `f7f4f55`, pushed, PR not yet opened): `ObsidianGateway` + `FlushCapturesUseCase` + `Capture` `pending-flush`/`flushed` states, built against the API Design section's proposed `obs write` contract. Re-verified live that `obs write`/`daily`/`template` are still unshipped (`obs` v4.3.0 returns `Unknown command: write`) — P0 remains the confirmed blocker. `atlas catch` write-through and `atlas flush` CLI wiring not yet done.
